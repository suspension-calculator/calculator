...
def test_link_weight_calculation(mock_output_process, mock_input_process):
    ...
    assert (
        abs(constant.sizing.link_weight[0] - expected_weight) < 0.001
...
def test_safety_factors(mock_output_process, mock_input_process):
    ...
    assert (
        constant.sizing.FS_buckling[0] > 0
    ), "Buckling safety factor should be positive"
    ...
    assert (
        abs(constant.sizing.FS_RE[0] - expected_RE_SF) < 0.001
