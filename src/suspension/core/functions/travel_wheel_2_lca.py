...
def wheel_2_lca(dZ, UA_Xi, UA_Zi, UF_X, UF_Z, LA_Xi, LA_Zi, LF_X, LF_Z, upperLength, lowerLength, sepAxle, sepFrame, wheel_Xi, wheel_Zi):
    ...
    while True:
        UA_Xf,UA_Zf,LA_Xf,LA_Zf = travel_solve(dZi, UA_Xi, UA_Zi, UF_X, UF_Z, LA_Xi, LA_Zi, LF_X, LF_Z, upperLength, lowerLength, sepAxle, sepFrame) # get travel points
        angle = pinion_angle_change(UA_Xi, UA_Zi, UA_Xf, UA_Zf, LA_Xi, LA_Zi, LA_Xf, LA_Zf)
        wheel_Zf = on_axle_movement(wheel_Xi, wheel_Zi, LA_Xi, LA_Zi, LA_Xf, LA_Zf,angle)[1] # get travel wheel center

        d_WC_z = wheel_Zf-wheel_Zi # dZ of wheel center from ride
        error = d_WC_z - dZ # difference between wheel center travel and desired wheel center travel
        if abs(error)<.0001:break # once error is low 0.0001", exit loop, loop is exited here so that dZi is at the low error
