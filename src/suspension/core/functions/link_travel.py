...
def travel_solve(
    ...
    if LA_Xi < LF_X:  # Determine if frame is in front of axle
        LA_Xf = LF_X - (
            (lowerLength**2 - (LA_Zf - LF_Z) ** 2) ** 0.5
        )  # calculate X distance between axle and frame on lower link, subtract from frame
    else:
        LA_Xf = LF_X + (
            (lowerLength**2 - (LA_Zf - LF_Z) ** 2) ** 0.5
        )  # calculate X distance between axle and frame on lower link, add to frame

    Angle_LL_Horiz = asin(
        (LF_Z - LA_Zf) / lowerLength
    )  # find the angle between lower link and horizontal
    Length_UF_LAF = dis2D(
        UF_X, UF_Z, LA_Xf, LA_Zf
    )  # find distance between upper frame and final lower axle
    Angle_LA_Opp_F = acos(
        (lowerLength**2 + Length_UF_LAF**2 - sepFrame**2)
        / (2 * lowerLength * Length_UF_LAF)
    )  # find angle between lower link and line from lower axle to upper frame
    Angle_LA_Opp_U = acos(
        (sepAxle**2 + Length_UF_LAF**2 - upperLength**2) / (2 * sepAxle * Length_UF_LAF)
    )  # find angle between axle points line and line between from lower axle to upper frame
    Angle_LL_Axle_Mounts = (
        Angle_LA_Opp_F + Angle_LA_Opp_U
    )  # find angle between lower link and axle points line
    Angle_Pinion_Line = (
        Angle_LL_Horiz + Angle_LL_Axle_Mounts
    )  # find angle between horizontal and axle points line
