...
class aboutPage(tk.Frame):
    def __init__(self,master):
        ...
        if True: # Page Selection
            ...
            button2 = tk.Button(calc_page_sel, text ="About", fg=buttonColor, bg=pressedButtonColor, command = lambda : master.switch_frame("about"),height= 1, width=8,font=(fontType,13))
        ...
        if True: # About Section
            label = tk.Label(self, text="Read Me",background=bgColor,foreground=entryTextColor,font=(fontType,20),anchor='w')
            label.grid(row=1, column = 0, pady=1)

            i_row=0

            label = tk.Label(infoFrame, text="The unit system can be selected under settings. The options are the Metric system and the Imperial system.",background=bgColor,foreground=entryTextColor,font=(fontType,15),anchor='w')
            label.grid(row=i_row, column = 0,sticky='w')
            i_row=i_row+1
            label = tk.Label(infoFrame, text="Other setting include number of travel points, front X inversion, and if tire loading should me simulated",background=bgColor,foreground=entryTextColor,font=(fontType,15),anchor='w')
            label.grid(row=i_row, column = 0,sticky='w')
            i_row=i_row+1

            label = tk.Label(infoFrame, text="",background=bgColor,foreground=entryTextColor,font=(fontType,15),anchor='w')
            label.grid(row=i_row, column = 0,sticky='w')
            i_row=i_row+1

            label = tk.Label(infoFrame, text="The link arrangement, at either end, can be:",background=bgColor,foreground=entryTextColor,font=(fontType,15),anchor='w')
            label.grid(row=i_row, column = 0,sticky='w')
            i_row=i_row+1
            label = tk.Label(infoFrame, text="4 link (2 upper + 2 lower), 3 link (1 upper + 2 lower) plus panhard, or 4 link (2 upper + 2 lower) plus panhard.",background=bgColor,foreground=entryTextColor,font=(fontType,15),anchor='w')
            label.grid(row=i_row, column = 0,sticky='w')
            i_row=i_row+1
            ...
            label = tk.Label(infoFrame, text="When there are more than 4 links, e.g. 4 link plus panhard (AKA 5 link) the suspension is said to be “over-constrained” and rigid links will prevent articulation.",background=bgColor,foreground=entryTextColor,font=(fontType,15),anchor='w')
            label.grid(row=i_row, column = 0,sticky='w')
            i_row=i_row+1
            label = tk.Label(infoFrame, text="Vehicles with factory fitted 4 link plus panhard suspension overcome the binding to some extent by making the upper and lower links close to parallel and using flexible bushes in the link joints.",background=bgColor,foreground=entryTextColor,font=(fontType,15),anchor='w')
            label.grid(row=i_row, column = 0,sticky='w')
            i_row=i_row+1
            label = tk.Label(infoFrame, text="Another problem with over-constrained suspension, e.g. radius arms, or 4 link plus panhard, is that some properties such as roll axis inclination can't easily be calculated with certainty.",background=bgColor,foreground=entryTextColor,font=(fontType,15),anchor='w')
            label.grid(row=i_row, column = 0,sticky='w')
            i_row=i_row+1
            label = tk.Label(infoFrame, text="The calculator doesn't care nor check for binding during suspension travel or articulation – user beware if you use this to simulate radius arm or 4 link plus panhard suspension.",background=bgColor,foreground=entryTextColor,font=(fontType,15),anchor='w')
            label.grid(row=i_row, column = 0,sticky='w')
            i_row=i_row+1
            ...
            label = tk.Label(infoFrame, text="The datum (0.00) for all x coordinates is the center of the rear axle, with x coordinates increasing in the direction from rear to front axle, i.e. positive to the right, negative to left of rear axle.",background=bgColor,foreground=entryTextColor,font=(fontType,15),anchor='w')
            label.grid(row=i_row, column = 0,sticky='w')
            i_row=i_row+1
            label = tk.Label(infoFrame, text="For convenience with front suspension data entry only, a pseudo X' coordinate has been added, where X' is measured in the x direction from the center of the front axle.",background=bgColor,foreground=entryTextColor,font=(fontType,15),anchor='w')
            label.grid(row=i_row, column = 0,sticky='w')
            i_row=i_row+1
            label = tk.Label(infoFrame, text="The user can choose to use the global sign convensions",background=bgColor,foreground=entryTextColor,font=(fontType,15),anchor='w')
            label.grid(row=i_row, column = 0,sticky='w')
            i_row=i_row+1
            label = tk.Label(infoFrame, text="The calculator will automatically calculate the true X coordinates by adding the wheelbase to the X' values.",background=bgColor,foreground=entryTextColor,font=(fontType,15),anchor='w')
            label.grid(row=i_row, column = 0,sticky='w')
            i_row=i_row+1
            label = tk.Label(infoFrame, text="The Driveshafts and Shocks also uses the pseudo X' coordinate.",background=bgColor,foreground=entryTextColor,font=(fontType,15),anchor='w')
            label.grid(row=i_row, column = 0,sticky='w')
            i_row=i_row+1
            ...
            label = tk.Label(infoFrame, text="On Link Calculator, the movement of points due to suspension travel are plotted with respect to the body being held fixed.",background=bgColor,foreground=entryTextColor,font=(fontType,15),anchor='w')
            label.grid(row=i_row, column = 0,sticky='w')
            i_row=i_row+1
            label = tk.Label(infoFrame, text="On Vehicle Pitch, links are ploted with respect to level ground.",background=bgColor,foreground=entryTextColor,font=(fontType,15),anchor='w')
            label.grid(row=i_row, column = 0,sticky='w')
            i_row=i_row+1
            ...
            label = tk.Label(infoFrame, text="Anti-squat for the rear suspension, and anti-lift for the front suspension is calculated for the case of forward acceleration.",background=bgColor,foreground=entryTextColor,font=(fontType,15),anchor='w')
            label.grid(row=i_row, column = 0,sticky='w')
            i_row=i_row+1
            label = tk.Label(infoFrame, text="Anti-dive for the front suspension, and anti-lift for the rear suspension is calculated for the case of braking.",background=bgColor,foreground=entryTextColor,font=(fontType,15),anchor='w')
            label.grid(row=i_row, column = 0,sticky='w')
            i_row=i_row+1
            ...
            label = tk.Label(infoFrame, text="If all of the drive is transmitted through the rear wheels (front drive bias = 0%), i.e. rear two wheel drive, anti-squat will be high and anti-lift at the front suspension will be zero.",background=bgColor,foreground=entryTextColor,font=(fontType,15),anchor='w')
            label.grid(row=i_row, column = 0,sticky='w')
            i_row=i_row+1
            label = tk.Label(infoFrame, text="If the drive is split/shared between rear and front wheels, i.e. four wheel drive, the value for anti-squat will reduce and anti-lift (front) will increase. For example, if the drive split is 50%/50%",background=bgColor,foreground=entryTextColor,font=(fontType,15),anchor='w')
            label.grid(row=i_row, column = 0,sticky='w')
            i_row=i_row+1
            label = tk.Label(infoFrame, text="  the anti-squat will be 0.50 times what it would be in two wheel drive, for the same suspension geometry – note earlier versions of the calculator assume front drive bias = 0% (two wheel drive).",background=bgColor,foreground=entryTextColor,font=(fontType,15),anchor='w')
            label.grid(row=i_row, column = 0,sticky='w')
            i_row=i_row+1
            ...
            label = tk.Label(infoFrame, text="If the transfer case has a non-biasing open center diff, the drive bias will be 50% to both the rear and front axles. If the center diff is locked, or the transfer case has no center diff the",background=bgColor,foreground=entryTextColor,font=(fontType,15),anchor='w')
            label.grid(row=i_row, column = 0,sticky='w')
            i_row=i_row+1
            label = tk.Label(infoFrame, text="  rear and front drive shafts will be forced to turn at the same speed and the drive bias will be governed by the rear/front traction. For example, if insignificant front traction, front drive bias can be 0%",background=bgColor,foreground=entryTextColor,font=(fontType,15),anchor='w')
            label.grid(row=i_row, column = 0,sticky='w')
            i_row=i_row+1

            label = tk.Label(infoFrame, text="The amount of weight/force transfer during acceleration/braking that induces squat/dive  = [acceleration x sprung mass x height of center of gravity of sprung mass / wheelbase]",background=bgColor,foreground=entryTextColor,font=(fontType,15),anchor='w')
            label.grid(row=i_row, column = 0,sticky='w')
            i_row=i_row+1
            label = tk.Label(infoFrame, text="The spreadsheet calculates sprung mass by subtracting the unsprung mass from the vehicle mass. The height of center of gravity of sprung mass is calculated using",background=bgColor,foreground=entryTextColor,font=(fontType,15),anchor='w')
            label.grid(row=i_row, column = 0,sticky='w')
            i_row=i_row+1
            label = tk.Label(infoFrame, text="  the height of the vehicle center of gravity, the vehicle mass, rear and front unsprung masses. Guessing or disregarding those values will affect the accuracy of some calculations, so use your best estimates.",background=bgColor,foreground=entryTextColor,font=(fontType,15),anchor='w')
            label.grid(row=i_row, column = 0,sticky='w')
            i_row=i_row+1
            ...
            label = tk.Label(infoFrame, text="For rear suspension links the forces are calculated assuming all of the drive force is transferred through the rear wheels (worse case).",background=bgColor,foreground=entryTextColor,font=(fontType,15),anchor='w')
            label.grid(row=i_row, column = 0,sticky='w')
            i_row=i_row+1
            label = tk.Label(infoFrame, text="For front suspension links the forces are calculated assuming all of the braking force is transferred through the front wheels.",background=bgColor,foreground=entryTextColor,font=(fontType,15),anchor='w')
            label.grid(row=i_row, column = 0,sticky='w')
            i_row=i_row+1
            label = tk.Label(infoFrame, text="The forces do not account for the change in force from the coilovers.",background=bgColor,foreground=entryTextColor,font=(fontType,15),anchor='w')
            label.grid(row=i_row, column = 0,sticky='w')
            i_row=i_row+1

            label = tk.Label(infoFrame, text="If you are only interested in the suspension at the rear axle (or only at the front axle), you only need to change the link geometry for the axle you are interested in",background=bgColor,foreground=entryTextColor,font=(fontType,15),anchor='w')
            label.grid(row=i_row, column = 0,sticky='w')
            i_row=i_row+1
            label = tk.Label(infoFrame, text="  and leave the link geometry for the other axle “as is”. That won't affect the results for the suspension you are interested in, but you should ignore results for the other axle and vehicle roll axis.",background=bgColor,foreground=entryTextColor,font=(fontType,15),anchor='w')
            label.grid(row=i_row, column = 0,sticky='w')
            i_row=i_row+1
            ...
            label = tk.Label(infoFrame, text="For simplicity of calculation, the nominated wheel travel, up (bump) or down (droop) is assumed to apply at the axle end of the lower links.",background=bgColor,foreground=entryTextColor,font=(fontType,15),anchor='w')
            label.grid(row=i_row, column = 0,sticky='w')
            i_row=i_row+1
            label = tk.Label(infoFrame, text="The error resulting from this assumption is negligible when thepinion rotation is small.",background=bgColor,foreground=entryTextColor,font=(fontType,15),anchor='w')
            label.grid(row=i_row, column = 0,sticky='w')
            i_row=i_row+1
            ...
            label = tk.Label(infoFrame, text="The effect of sideways motion from the panhard bar is not accounted for in squat, dive, lift, and spring rate calculations. ",background=bgColor,foreground=entryTextColor,font=(fontType,15),anchor='w')
            label.grid(row=i_row, column = 0,sticky='w')
            i_row=i_row+1
            ...
            label = tk.Label(infoFrame, text="Coilover can be used to find installation ratios and mounting location for air shocks and struts. Information related to the springs should be disregarded in such cases.",background=bgColor,foreground=entryTextColor,font=(fontType,15),anchor='w')
            label.grid(row=i_row, column = 0,sticky='w')
            i_row=i_row+1

        if True: # Version History
            label = tk.Label(self, text="Version History",background=bgColor,foreground=entryTextColor,font=(fontType,15),anchor='w')
            label.grid(row=1, column = 2, pady=1)

            revisionArea = st.ScrolledText(RevisionHistoryFrame, width=70,height=70,font=(fontType,15),bg=bgColor,fg=entryTextColor,relief="flat")
            revisionArea.grid(row=0,column=0)

            revisionArea.insert(tk.INSERT,
                                """\
1.5b     2003.12.05
...
Revision by Treefrog
    ...
    Consolidated Read Me, Coordinates, and Reversion to About
        ...
        if True: # Coordinate System Images
            label = tk.Label(self, text="Coordinate System",background=bgColor,foreground=entryTextColor,font=(fontType,15),anchor='w')
            ...
            if True: # Top view coordinate system
                ...
                axTopCoord.plot(circleX*.4,circleY*.4,color=entryTextColor)
                axTopCoord.plot(circleX*.4+5,circleY*.4+.5,color=entryTextColor)
                axTopCoord.plot([1,1,-1,-1,1],[2,1.2,1.2,2,2],color=entryTextColor)
                axTopCoord.plot([1,1,-1,-1,1],[-2,-1.2,-1.2,-2,-2],color=entryTextColor)
                axTopCoord.plot([6,6,4,4,6],[2,1.2,1.2,2,2],color=entryTextColor)
                axTopCoord.plot([6,6,4,4,6],[-2,-1.2,-1.2,-2,-2],color=entryTextColor)
                axTopCoord.plot([-.1,-.1],[-1.2,1.2],color=entryTextColor)
                axTopCoord.plot([.1,.1],[-1.2,1.2],color=entryTextColor)
                axTopCoord.plot([4.9,4.9],[-1.2,1.2],color=entryTextColor)
                axTopCoord.plot([5.1,5.1],[-1.2,1.2],color=entryTextColor)

                axTopCoord.plot([.05,1],[0,0],color="#CF0000",linewidth=2)
                axTopCoord.plot([.93,1],[-.07,0],color="#CF0000",linewidth=3)
                axTopCoord.plot([.93,1],[.07,0],color="#CF0000",linewidth=3)
                axTopCoord.text(1.1,0,"X",color="#CF0000")
                axTopCoord.plot([0,0],[.05,1],color="#00FF00",linewidth=2)
                axTopCoord.plot([.07,0],[.93,1],color="#00FF00",linewidth=2)
                axTopCoord.plot([-.07,0],[.93,1],color="#00FF00",linewidth=2)
                axTopCoord.text(.15,.9,"Y",color="#00FF00")
            ...
            if True: # Side View coordinate system
                ...
                axSideCoord.plot(1.1*circleX[0:179],1.1*circleY[0:179]+1,color=entryTextColor)
                axSideCoord.plot([1.1,3.9],[1,1],color=entryTextColor)
                axSideCoord.plot(1.1*circleX[0:179]+5,1.1*circleY[0:179]+1,color=entryTextColor)

                axSideCoord.plot([.05,1],[0,0],color="#CF0000",linewidth=2)
                axSideCoord.plot([.93,1],[-.07,0],color="#CF0000",linewidth=3)
                axSideCoord.plot([.93,1],[.07,0],color="#CF0000",linewidth=3)
                axSideCoord.text(1.1,0,"X",color="#CF0000")
                axSideCoord.plot([0,0],[.05,1],color="#0000FF",linewidth=2)
                axSideCoord.plot([.07,0],[.93,1],color="#0000FF",linewidth=2)
                axSideCoord.plot([-.07,0],[.93,1],color="#0000FF",linewidth=2)
