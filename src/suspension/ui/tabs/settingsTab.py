...
class settingsPage(tk.Frame):
    def __init__(self,master):
        ...
        if True: # Page Selection
            ...
            button2 = tk.Button(calc_page_sel, text ="Settings", fg=buttonColor, bg=pressedButtonColor, command = lambda : master.switch_frame("settings"),height= 1, width=8,font=(fontType,13))
        ...
        if True: # Settings Frame
            ...
            label = tk.Label(settings, text="Unit System:",background=bgColor,foreground=entryTextColor,font=(fontType,17), justify=tk.RIGHT)
            ...
            label = tk.Label(settings, text="Invert front X?",background=bgColor,foreground=entryTextColor,font=(fontType,17), justify=tk.RIGHT)
            label.grid(row=i_row, column = 0,sticky='e')
            label.bind("<1>", lambda event: update_outputs())
            ...
            invert_front_menu.nametowidget(invert_front_menu.menuname).config(font=(fontType,17))

            i_row = 2

            label = tk.Label(settings, text="Model tire loading?",background=bgColor,foreground=entryTextColor,font=(fontType,17), justify=tk.RIGHT)
            label.grid(row=i_row, column = 0,sticky='e')
            label.bind("<1>", lambda event: update_outputs())
            ...
            tire_loading_menu.nametowidget(tire_loading_menu.menuname).config(font=(fontType,17))

            i_row = 3

            label = tk.Label(settings, text="Sample points:",background=bgColor,foreground=entryTextColor,font=(fontType,17), justify=tk.RIGHT)
            label.grid(row=i_row, column = 0,sticky='e')
            label.bind("<1>", lambda event: update_outputs())

            text = tk.StringVar()
            text.set(S.sample_points)
            sample_points = tk.Entry(settings,font=(fontType,17), justify=tk.CENTER,width=4, textvariable=text,insertbackground=entryTextColor)
