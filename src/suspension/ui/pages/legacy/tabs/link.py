# src/suspension/ui/tabs_original/link.py
...
from suspension.core.services.calculations import run_link_calc
from suspension.io import S, PS, x, y, z
from suspension.io import load_susp
from suspension.io import save_susp, save_as_susp
...
class linkPage(tk.Frame):
    def __init__(self, master):
        ...
        plt.rcParams["figure.subplot.hspace"] = 0.11
        ...
        if True:  # Front Inputs
            ...
            F_UL_Count_menu.nametowidget(F_UL_Count_menu.menuname).config(
                font=(fontType, 17)
            )
            ...
            F_P_exist_menu.nametowidget(F_P_exist_menu.menuname).config(
                font=(fontType, 17)
            )
        ...
        if True:  # Rear Inputs
            ...
            R_UL_Count_menu.nametowidget(R_UL_Count_menu.menuname).config(
                font=(fontType, 17)
            )
            ...
            R_P_exist_menu.nametowidget(R_P_exist_menu.menuname).config(
                font=(fontType, 17)
            )
        ...
        if True:  # Vehicle Inputs
            ...
            label = tk.Label(
                calc_vehicle_inputs,
                text="Wheelbase [{}]:".format(S.position_units),
                background=bgColor,
                foreground=entryTextColor,
                font=(fontType, 17),
                justify=tk.RIGHT,
            ...
            label = tk.Label(
                calc_vehicle_inputs,
                text="Vehicle Mass [{}]:".format(S.mass_units),
                background=bgColor,
                foreground=entryTextColor,
                font=(fontType, 17),
                justify=tk.RIGHT,
            ...
            label = tk.Label(
                calc_vehicle_inputs,
                text="Vehicle CG Height [{}]:".format(S.position_units),
                background=bgColor,
                foreground=entryTextColor,
                font=(fontType, 17),
                justify=tk.RIGHT,
            ...
            label = tk.Label(
                calc_vehicle_inputs,
                text="Ride Anti Lines ",
                background=rideAntiColor,
                foreground=textEntryColor,
                font=(fontType, 17),
                anchor="e",
            )
            ...
            ps_converge_menu = tk.OptionMenu(
                calc_vehicle_inputs, ps_converge, "Show", "Hide"
            )
            ...
            ps_converge_menu.nametowidget(ps_converge_menu.menuname).config(
                font=(fontType, 17)
            )
            ps_IC_move = tk.StringVar(calc_vehicle)
            ps_IC_move.set("Show")
            ps_IC_move_menu = tk.OptionMenu(
                calc_vehicle_inputs, ps_IC_move, "Show", "Hide"
            )
            ...
            ps_IC_move_menu.nametowidget(ps_IC_move_menu.menuname).config(
                font=(fontType, 17)
            )
            ps_ride_IC = tk.StringVar(calc_vehicle)
            ps_ride_IC.set("Show")
            ps_ride_IC_menu = tk.OptionMenu(
                calc_vehicle_inputs, ps_ride_IC, "Show", "Hide"
            )
            ...
            ps_ride_IC_menu.nametowidget(ps_ride_IC_menu.menuname).config(
                font=(fontType, 17)
            )
            ps_axle_roll = tk.StringVar(calc_vehicle)
            ps_axle_roll.set("Show")
            ps_axle_roll_menu = tk.OptionMenu(
                calc_vehicle_inputs, ps_axle_roll, "Show", "Hide"
            )
            ...
            ps_axle_roll_menu.nametowidget(ps_axle_roll_menu.menuname).config(
                font=(fontType, 17)
            )
            ps_roll_center = tk.StringVar(calc_vehicle)
            ps_roll_center.set("Show")
            ps_roll_center_menu = tk.OptionMenu(
                calc_vehicle_inputs, ps_roll_center, "Show", "Hide"
            )
            ...
            ps_roll_center_menu.nametowidget(ps_roll_center_menu.menuname).config(
                font=(fontType, 17)
            )
            ps_body_roll = tk.StringVar(calc_vehicle)
            ps_body_roll.set("Show")
            ps_body_roll_menu = tk.OptionMenu(
                calc_vehicle_inputs, ps_body_roll, "Show", "Hide"
            )
            ...
            ps_body_roll_menu.nametowidget(ps_body_roll_menu.menuname).config(
                font=(fontType, 17)
            )
            ps_100_anti = tk.StringVar(calc_vehicle)
            ps_100_anti.set("Show")
            ps_100_anti_menu = tk.OptionMenu(
                calc_vehicle_inputs, ps_100_anti, "Show", "Hide"
            )
            ...
            ps_100_anti_menu.nametowidget(ps_100_anti_menu.menuname).config(
                font=(fontType, 17)
            )
            ps_ride_anti = tk.StringVar(calc_vehicle)
            ps_ride_anti.set("Show")
            ps_ride_anti_menu = tk.OptionMenu(
                calc_vehicle_inputs, ps_ride_anti, "Show", "Hide"
            )
            ...
            ps_ride_anti_menu.nametowidget(ps_ride_anti_menu.menuname).config(
                font=(fontType, 17)
            )
        ...
        if True:  # Vehicle Outputs
            ...
            if True:  # Vehicle Outputs
                label = tk.Label(
                    calc_outputs,
                    text="Sprung Mass Center of Gravity",
                    background=bgColor,
                    foreground=entryTextColor,
                    font=(fontType, 20),
                    justify=tk.CENTER,
            ...
            if True:  # Rear Text Outputs
                ...
                label_R_converge.grid(
                    row=i_row + 7, column=i_column + 2, columnspan=2, sticky="w"
                )
        ...
        if True:  # Setup Plot Areas
            ...
            self.figF.subplots_adjust(hspace=0.25)
            self.figF.subplots_adjust(wspace=0.2)
        ...
        def update_outputs(*args):
            ...
            if plots_changed:
                ...
                if True:  # Plot Front
                    if True:  # Antis
                        self.axF[0, 0].plot(
                            outputs.F.Anti_Dive, outputs.F.Travel, color=rideAntiColor
                        )
                        self.axF[0, 0].plot(
                            outputs.F.Anti_Lift,
                            outputs.F.Travel,
                        ...
                        if inputs.V.brake_bias > inputs.V.drive_bias:
                            ...
                            temp2 = "left"
                        self.axF[0, 0].text(
                            outputs.F.Anti_Dive[0],
                            outputs.F.Travel[0],
                            "{:.1f}%".format(outputs.F.Anti_Dive[0]),
                            color=rideAntiColor,
                            horizontalalignment=temp1,
                        )
                        self.axF[0, 0].text(
                            outputs.F.Anti_Lift[0],
                            outputs.F.Travel[0],
                            "{:.1f}%".format(outputs.F.Anti_Lift[0]),
                            color=plotSecondaryColor,
                            horizontalalignment=temp2,
                        )
                        self.axF[0, 0].text(
                            outputs.F.Anti_Dive[S.sample_points],
                        ...
                        self.axF[0, 0].text(
                            outputs.F.Anti_Lift[S.sample_points],
                            outputs.F.Travel[S.sample_points],
                            "{:.1f}%\nAntilift".format(
                                outputs.F.Anti_Lift[S.sample_points]
                            ),
                        ...
                        self.axF[0, 0].text(
                            outputs.F.Anti_Dive[S.sample_points * 2],
                            outputs.F.Travel[S.sample_points * 2],
                            "{:.1f}%".format(outputs.F.Anti_Dive[S.sample_points * 2]),
                            color=rideAntiColor,
                            horizontalalignment=temp1,
                            va="top",
                        )
                        self.axF[0, 0].text(
                            outputs.F.Anti_Lift[S.sample_points * 2],
                            outputs.F.Travel[S.sample_points * 2],
                            "{:.1f}%".format(outputs.F.Anti_Lift[S.sample_points * 2]),
                            color=plotSecondaryColor,
                            horizontalalignment=temp2,
                            va="top",
                        )
                        self.axF[0, 0].set_xlim(
                            [
                                min(min(outputs.F.Anti_Lift), min(outputs.F.Anti_Dive))
                                - 5,
                                max(max(outputs.F.Anti_Lift), max(outputs.F.Anti_Dive))
                                + 5,
                            ]
                        )
                        self.axF[0, 0].set_ylim([inputs.F.droop, inputs.F.bump])
                        self.axF[0, 0].set_xlabel("Antis")
                        self.axF[0, 0].set_ylabel(
                    ...
                    if True:  # Roll slope
                        self.axF[0, 1].plot(
                            -outputs.F.Roll_Slope, outputs.F.Travel, color=rollColor
                        )
                        self.axF[0, 1].text(
                            -outputs.F.Roll_Slope[0],
                            outputs.F.Travel[0],
                            "{:.1f}".format(-outputs.F.Roll_Slope[0]),
                            color=rollColor,
                        )
                        self.axF[0, 1].text(
                            -outputs.F.Roll_Slope[S.sample_points],
                            outputs.F.Travel[S.sample_points],
                            "{:.1f}".format(-outputs.F.Roll_Slope[S.sample_points]),
                            color=rollColor,
                        )
                        self.axF[0, 1].text(
                            -outputs.F.Roll_Slope[S.sample_points * 2],
                            outputs.F.Travel[S.sample_points * 2],
                            "{:.1f}".format(-outputs.F.Roll_Slope[S.sample_points * 2]),
                            color=rollColor,
                            va="top",
                        )
                        self.axF[0, 1].set_xlim(
                            [
                                min(-outputs.F.Roll_Slope) - 1,
                                max(-outputs.F.Roll_Slope) + 1,
                            ]
                        )
                        self.axF[0, 1].set_ylim([inputs.F.droop, inputs.F.bump])
                        self.axF[0, 1].set_xlabel("Roll Slope [deg]")
                        self.axF[0, 1].set_ylabel(
                            "Travel [{}]".format(S.position_units)
                        )
                    ...
                    if True:  # Roll Center
                        self.axF[1, 1].plot(
                            outputs.F.Roll_Center, outputs.F.Travel, color=rollColor
                        )
                        self.axF[1, 1].text(
                            outputs.F.Roll_Center[0],
                            outputs.F.Travel[0],
                            "{:.1f}".format(outputs.F.Roll_Center[0]),
                            color=rollColor,
                        )
                        self.axF[1, 1].text(
                            outputs.F.Roll_Center[S.sample_points],
                            outputs.F.Travel[S.sample_points],
                            "{:.1f}".format(outputs.F.Roll_Center[S.sample_points]),
                            color=rollColor,
                        )
                        self.axF[1, 1].text(
                            outputs.F.Roll_Center[S.sample_points * 2],
                            outputs.F.Travel[S.sample_points * 2],
                            "{:.1f}".format(outputs.F.Roll_Center[S.sample_points * 2]),
                            color=rollColor,
                            va="top",
                        )
                        self.axF[1, 1].set_xlim(
                            [
                                min(outputs.F.Roll_Center) - 1,
                                max(outputs.F.Roll_Center) + 1,
                            ]
                        )
                        self.axF[1, 1].set_ylim([inputs.F.droop, inputs.F.bump])
                        self.axF[1, 1].set_xlabel(
                            "Roll Center [{}]".format(S.position_units)
                        )
                        self.axF[1, 1].set_ylabel(
                            "Travel [{}]".format(S.position_units)
                        )
                    ...
                    if True:  # Pinion Change
                        self.axF[1, 0].plot(
                            -outputs.F.Pinion_Change,
                            outputs.F.Travel,
                            color=plotMainColor,
                        )
                        self.axF[1, 0].text(
                            -outputs.F.Pinion_Change[0],
                            outputs.F.Travel[0],
                            "{:.1f}".format(-outputs.F.Pinion_Change[0]),
                            color=plotMainColor,
                        )
                        self.axF[1, 0].text(
                        ...
                        self.axF[1, 0].set_xlim(
                            [
                                min(-outputs.F.Pinion_Change) - 1,
                                max(-outputs.F.Pinion_Change) + 1,
                            ]
                        )
                        self.axF[1, 0].set_ylim([inputs.F.droop, inputs.F.bump])
                        self.axF[1, 0].set_xlabel("Pinion Change [deg]")
                        self.axF[1, 0].set_ylabel(
                            "Travel [{}]".format(S.position_units)
                        )
                    ...
                    for i_x in range(2):
                        for i_y in range(2):
                            ...
                            self.axF[i_x, i_y].spines[
                                ["top", "bottom", "left", "right"]
                            ].set_color(entryTextColor)
                            self.axF[i_x, i_y].tick_params(
                    ...
                    c1y = inputs.F.tire_radius + 0.5 * inputs.F.tire_diameter
                    ...
                    c4y = inputs.F.tire_radius + 0.5 * inputs.F.tire_diameter
                    self.axF_view.plot(
                        [c1x, c4x], [c1y, c4y], color=vehicleColor
                    )  # Pos y tire top line
                    self.axF_view.plot(
                        [c2x, c3x], [c2y, c3y], color=vehicleColor
                    )  # Pos y tire bottom line
                    self.axF_view.plot(
                        [c1x, c2x], [c1y, c2y], color=vehicleColor
                    )  # Pos y tire pos y line
                    self.axF_view.plot(
                        [c3x, c4x], [c3y, c4y], color=vehicleColor
                    )  # Pos y tire neg y line
                    ...
                    self.axF_view.plot(
                        [c2x, c3x], [c2y, c3y], color=vehicleColor
                    )  # Neg y tire bottom line
                    self.axF_view.plot(
                        [c1x, c2x], [c1y, c2y], color=vehicleColor
                    )  # Neg y tire pos y line
                    self.axF_view.plot(
                        [c3x, c4x], [c3y, c4y], color=vehicleColor
                    )  # Neg y tire neg y line
                    ...
                    self.axF_view.plot(
                        [x1, x2], [y1, y2], color=vehicleColor
                    )  # Upper axle tube
                    x1 = inputs.F.track_width / 2 - inputs.F.tire_width / 2
                    x2 = -inputs.F.track_width / 2 + inputs.F.tire_width / 2
                    y1 = inputs.F.tire_radius - inputs.F.axle_tube / 2
                    ...
                    if inputs.F.panhard:
                        ...
                        self.axF_view.plot(
                            [x1, x2], [y1, y2], linewidth=linkLine, color=panhardColor
                        )  # Panhard ride
                        x1 = outputs.F.PF[y]
                        ...
                        self.axF_view.plot(
                            [x1, x2], [y1, y2], linewidth=travelLine, color=panhardColor
                        )  # Panhard bump
                        x1 = outputs.F.PF[y]
                        x2 = outputs.F.PA[0, y]
                        y1 = outputs.F.PF[z]
                        ...
                        self.axF_view.plot(
                            ...
                            outputs.F.PA[:, z],
                            linestyle="dotted",
                            linewidth=dottedLine,
                            color=panhardColor,
                        )  # Panhard arc

                    self.axF_view.axis("equal")
                    self.axF_view.set_xlim(
                        [
                            -1.1 * inputs.F.track_width / 2 - inputs.F.tire_width,
                            1.1 * inputs.F.track_width / 2 + inputs.F.tire_width,
                        ]
                    )
                    ...
                    self.axF_view.spines[["top", "bottom", "left", "right"]].set_color(
                        entryTextColor
                    )
                    self.axF_view.tick_params(axis="x", colors=entryTextColor)
                    self.axF_view.tick_params(axis="y", colors=entryTextColor)

                    self.axF_view.set_title(
                        "View From Front", color=entryTextColor, y=1.0, pad=-23
                    )

                if True:  # Plot Rear
                    if True:  # Antis
                        self.axR[0, 0].plot(
                            outputs.R.Anti_Squat, outputs.R.Travel, color=rideAntiColor
                        )
                        self.axR[0, 0].plot(
                            outputs.R.Anti_Lift,
                            outputs.R.Travel,
                        ...
                        if inputs.V.brake_bias > inputs.V.drive_bias:
                            ...
                            temp2 = "left"
                        self.axR[0, 0].text(
                            outputs.R.Anti_Squat[0],
                            outputs.R.Travel[0],
                            "{:.1f}%".format(outputs.R.Anti_Squat[0]),
                            color=rideAntiColor,
                            horizontalalignment=temp1,
                        )
                        self.axR[0, 0].text(
                            outputs.R.Anti_Lift[0],
                            outputs.R.Travel[0],
                            "{:.1f}%".format(outputs.R.Anti_Lift[0]),
                            color=plotSecondaryColor,
                            horizontalalignment=temp2,
                        )
                        self.axR[0, 0].text(
                            outputs.R.Anti_Squat[S.sample_points],
                        ...
                        self.axR[0, 0].text(
                            outputs.R.Anti_Lift[S.sample_points],
                            outputs.R.Travel[S.sample_points],
                            "{:.1f}%\nAntilift".format(
                                outputs.R.Anti_Lift[S.sample_points]
                            ),
                        ...
                        self.axR[0, 0].text(
                            outputs.R.Anti_Squat[S.sample_points * 2],
                            outputs.R.Travel[S.sample_points * 2],
                            "{:.1f}%".format(outputs.R.Anti_Squat[S.sample_points * 2]),
                            color=rideAntiColor,
                            horizontalalignment=temp1,
                            va="top",
                        )
                        self.axR[0, 0].text(
                            outputs.R.Anti_Lift[S.sample_points * 2],
                            outputs.R.Travel[S.sample_points * 2],
                            "{:.1f}%".format(outputs.R.Anti_Lift[S.sample_points * 2]),
                            color=plotSecondaryColor,
                            horizontalalignment=temp2,
                            va="top",
                        )
                        self.axR[0, 0].set_xlim(
                            [
                                min(min(outputs.R.Anti_Lift), min(outputs.R.Anti_Squat))
                                - 5,
                                max(max(outputs.R.Anti_Lift), max(outputs.R.Anti_Squat))
                                + 5,
                            ]
                        )
                        self.axR[0, 0].set_ylim([inputs.R.droop, inputs.R.bump])
                        self.axR[0, 0].set_xlabel("Antis")
                        self.axR[0, 0].set_ylabel(
                            "Travel [{}]".format(S.position_units)
                        )
                    ...
                    if True:  # Roll slope
                        self.axR[0, 1].plot(
                            outputs.R.Roll_Slope, outputs.R.Travel, color=rollColor
                        )
                        self.axR[0, 1].text(
                            outputs.R.Roll_Slope[0],
                            outputs.R.Travel[0],
                            "{:.1f}".format(outputs.R.Roll_Slope[0]),
                            color=rollColor,
                        )
                        self.axR[0, 1].text(
                            outputs.R.Roll_Slope[S.sample_points],
                            outputs.R.Travel[S.sample_points],
                            "{:.1f}".format(outputs.R.Roll_Slope[S.sample_points]),
                            color=rollColor,
                        )
                        self.axR[0, 1].text(
                            outputs.R.Roll_Slope[S.sample_points * 2],
                            outputs.R.Travel[S.sample_points * 2],
                            "{:.1f}".format(outputs.R.Roll_Slope[S.sample_points * 2]),
                            color=rollColor,
                            va="top",
                        )
                        self.axR[0, 1].set_xlim(
                            [
                                min(outputs.R.Roll_Slope) - 1,
                                max(outputs.R.Roll_Slope) + 1,
                            ]
                        )
                        self.axR[0, 1].set_ylim([inputs.R.droop, inputs.R.bump])
                        self.axR[0, 1].set_xlabel("Roll Slope [deg]")
                        self.axR[0, 1].set_ylabel(
                            "Travel [{}]".format(S.position_units)
                        )
                    ...
                    if True:  # Roll Center
                        self.axR[1, 1].plot(
                            outputs.R.Roll_Center, outputs.R.Travel, color=rollColor
                        )
                        self.axR[1, 1].text(
                            outputs.R.Roll_Center[0],
                            outputs.R.Travel[0],
                            "{:.1f}".format(-outputs.R.Roll_Center[0]),
                            color=rollColor,
                        )
                        self.axR[1, 1].text(
                            outputs.R.Roll_Center[S.sample_points],
                            outputs.R.Travel[S.sample_points],
                            "{:.1f}".format(outputs.R.Roll_Center[S.sample_points]),
                            color=rollColor,
                        )
                        self.axR[1, 1].text(
                            outputs.R.Roll_Center[S.sample_points * 2],
                            outputs.R.Travel[S.sample_points * 2],
                            "{:.1f}".format(outputs.R.Roll_Center[S.sample_points * 2]),
                            color=rollColor,
                            va="top",
                        )
                        self.axR[1, 1].set_xlim(
                            [
                                min(outputs.R.Roll_Center) - 1,
                                max(outputs.R.Roll_Center) + 1,
                            ]
                        )
                        self.axR[1, 1].set_ylim([inputs.R.droop, inputs.R.bump])
                        self.axR[1, 1].set_xlabel(
                            "Roll Center [{}]".format(S.position_units)
                        )
                        self.axR[1, 1].set_ylabel(
                            "Travel [{}]".format(S.position_units)
                        )
                    ...
                    if True:  # Pinion Change
                        self.axR[1, 0].plot(
                            outputs.R.Pinion_Change,
                            outputs.R.Travel,
                            color=plotMainColor,
                        )
                        self.axR[1, 0].text(
                            outputs.R.Pinion_Change[0],
                            outputs.R.Travel[0],
                            "{:.1f}".format(outputs.R.Pinion_Change[0]),
                            color=plotMainColor,
                        )
                        self.axR[1, 0].text(
                        ...
                        self.axR[1, 0].set_xlim(
                            [
                                min(outputs.R.Pinion_Change) - 1,
                                max(outputs.R.Pinion_Change) + 1,
                            ]
                        )
                        self.axR[1, 0].set_ylim([inputs.R.droop, inputs.R.bump])
                        self.axR[1, 0].set_xlabel("Pinion Change [deg]")
                        self.axR[1, 0].set_ylabel(
                            "Travel [{}]".format(S.position_units)
                        )
                    ...
                    for i_x in range(2):
                        for i_y in range(2):
                            ...
                            self.axR[i_x, i_y].spines[
                                ["top", "bottom", "left", "right"]
                            ].set_color(entryTextColor)
                            self.axR[i_x, i_y].tick_params(
                                axis="x", colors=entryTextColor
                            )
                            self.axR[i_x, i_y].tick_params(
                                axis="y", colors=entryTextColor
                    ...
                    c1y = inputs.R.tire_radius + 0.5 * inputs.R.tire_diameter
                    ...
                    c4y = inputs.R.tire_radius + 0.5 * inputs.R.tire_diameter
                    self.axR_view.plot(
                        [c1x, c4x], [c1y, c4y], color=vehicleColor
                    )  # Pos y tire top line
                    self.axR_view.plot(
                    ...
                    self.axR_view.set_xlim(
                        [
                            -1.1 * inputs.R.track_width / 2 - inputs.R.tire_width,
                            1.1 * inputs.R.track_width / 2 + inputs.R.tire_width,
                        ]
                    )
                    ...
                    self.axR_view.spines[["top", "bottom", "left", "right"]].set_color(
                        entryTextColor
                    )
                    self.axR_view.tick_params(axis="x", colors=entryTextColor)
                    self.axR_view.tick_params(axis="y", colors=entryTextColor)

                    self.axR_view.set_title(
                        "View From Rear", color=entryTextColor, y=1.0, pad=-23
                    )
                ...
                if True:  # Plot Vehicle
                    if True:  # Top View
                        ...
                        self.axV[0, 0].plot(
                            [c2x, c3x], [c2y, c3y], color=vehicleColor
                        )  # Front pos y tire pos y line
                        self.axV[0, 0].plot(
                            [c1x, c4x], [c1y, c4y], color=vehicleColor
                        )  # Front pos y tire neg y line
                        self.axV[0, 0].plot(
                            [c1x, c2x], [c1y, c2y], color=vehicleColor
                        )  # Front pos y tire front line
                        self.axV[0, 0].plot(
                            [c3x, c4x], [c3y, c4y], color=vehicleColor
                        )  # Front pos y tire rear line
                        ...
                        self.axV[0, 0].plot(
                            [c2x, c3x], [c2y, c3y], color=vehicleColor
                        )  # Front neg y tire pos y line
                        self.axV[0, 0].plot(
                            [c1x, c4x], [c1y, c4y], color=vehicleColor
                        )  # Front neg y tire neg y line
                        self.axV[0, 0].plot(
                            [c1x, c2x], [c1y, c2y], color=vehicleColor
                        )  # Front neg y tire front line
                        self.axV[0, 0].plot(
                            [c3x, c4x], [c3y, c4y], color=vehicleColor
                        )  # Front neg y tire rear line
                        ...
                        self.axV[0, 0].plot(
                            [c2x, c3x], [c2y, c3y], color=vehicleColor
                        )  # Rear pos y tire pos y line
                        self.axV[0, 0].plot(
                            [c1x, c4x], [c1y, c4y], color=vehicleColor
                        )  # Rear pos y tire neg y line
                        self.axV[0, 0].plot(
                            [c1x, c2x], [c1y, c2y], color=vehicleColor
                        )  # Rear pos y tire front line
                        self.axV[0, 0].plot(
                            [c3x, c4x], [c3y, c4y], color=vehicleColor
                        )  # Rear pos y tire rear line
                        ...
                        self.axV[0, 0].plot(
                            [c2x, c3x], [c2y, c3y], color=vehicleColor
                        )  # Rear neg y tire pos y line
                        self.axV[0, 0].plot(
                            [c1x, c4x], [c1y, c4y], color=vehicleColor
                        )  # Rear neg y tire neg y line
                        self.axV[0, 0].plot(
                            [c1x, c2x], [c1y, c2y], color=vehicleColor
                        )  # Rear neg y tire front line
                        self.axV[0, 0].plot(
                            [c3x, c4x], [c3y, c4y], color=vehicleColor
                        )  # Rear neg y tire rear line
                        ...
                        y2 = outputs.F.LF[y]
                        self.axV[0, 0].plot(
                            [x1, x2], [y1, y2], color=lowerColor, linewidth=linkLine
                        )  # Front poy y lower link
                        x1 = outputs.F.LA[S.sample_points][x]
                        x2 = outputs.F.LF[x]
                        y1 = -outputs.F.LA[S.sample_points][y]
                        y2 = -outputs.F.LF[y]
                        self.axV[0, 0].plot(
                            [x1, x2], [y1, y2], color=lowerColor, linewidth=linkLine
                        )  # Front neg y lower link
                        x1 = outputs.F.UA[S.sample_points][x]
                        x2 = outputs.F.UF[x]
                        y1 = outputs.F.UA[S.sample_points][y]
                        y2 = outputs.F.UF[y]
                        self.axV[0, 0].plot(
                            [x1, x2], [y1, y2], color=upperColor, linewidth=linkLine
                        )  # Front pos y upper link
                        ...
                        if PS.converge == "Show":
                            ...
                            self.axV[0, 0].plot(
                                ...
                                linestyle="dotted",
                                linewidth=dottedLine,
                            )  # Front pos y lower link extended
                            x1 = outputs.F.LF[x]
                            x2 = outputs.F.L_converge_point[x]
                            y1 = -outputs.F.LF[y]
                            y2 = 0
                            self.axV[0, 0].plot(
                                ...
                                linestyle="dotted",
                                linewidth=dottedLine,
                            )  # Front neg y lower link extended
                            x1 = outputs.R.LF[x]
                            ...
                            self.axV[0, 0].plot(
                                ...
                                linestyle="dotted",
                                linewidth=dottedLine,
                            )  # Rear pos y lower link extended
                            x1 = outputs.R.LF[x]
                            x2 = outputs.R.L_converge_point[x]
                            y1 = -outputs.R.LF[y]
                            y2 = 0
                            self.axV[0, 0].plot(
                                ...
                                linestyle="dotted",
                                linewidth=dottedLine,
                            )  # Rear neg y lower link extended
                            if inputs.F.U_count == 2 and not inputs.F.panhard:
                                ...
                                self.axV[0, 0].plot(
                                    ...
                                    linestyle="dotted",
                                    linewidth=dottedLine,
                                )  # Front pos y upper link extended
                                x1 = outputs.F.UF[x]
                                x2 = outputs.F.U_converge_point[x]
                                y1 = -outputs.F.UF[y]
                                y2 = 0
                                self.axV[0, 0].plot(
                                    ...
                                    linestyle="dotted",
                                    linewidth=dottedLine,
                                )  # Front neg y upper link extended
                        ...
                        self.axV[0, 0].set_xlim(
                            [
                                -1.1 * inputs.R.tire_diameter / 2,
                                1.1 * inputs.F.tire_diameter / 2 + inputs.V.wheelbase,
                            ]
                        )
                        self.axV[0, 0].set_ylim(
                            [
                                -1.1
                                * (
                                    max(inputs.F.track_width, inputs.R.track_width) / 2
                                    + max(inputs.F.tire_width, inputs.R.tire_width) / 2
                                ),
                                1.1
                                * (
                                    max(inputs.F.track_width, inputs.R.track_width) / 2
                    ...
                    if True:  # Ride Height side view
                        ...
                        circle_x = (
                            inputs.F.axle_tube / 2 * np.cos(circle_angle)
                            + inputs.V.wheelbase
                        )
                        circle_y = (
                            inputs.F.axle_tube / 2 * np.sin(circle_angle)
                            + inputs.F.tire_radius
                        )
                        self.axV[1, 0].plot(
                            circle_x, circle_y, color=vehicleColor
                        )  # Front axle tube
                        circle_x = (
                            inputs.F.tire_diameter / 2 * np.cos(circle_angle)
                            + inputs.V.wheelbase
                        )
                        circle_y = (
                            inputs.F.tire_diameter / 2 * np.sin(circle_angle)
                            + inputs.F.tire_radius
                        )
                        ...
                        circle_y = (
                            inputs.R.axle_tube / 2 * np.sin(circle_angle)
                            + inputs.R.tire_radius
                        )
                        self.axV[1, 0].plot(
                            circle_x, circle_y, color=vehicleColor
                        )  # Rear axle tube
                        ...
                        circle_y = (
                            inputs.R.tire_diameter / 2 * np.sin(circle_angle)
                            + inputs.R.tire_radius
                        )
                        circle_y[circle_y < 0] = 0
                        self.axV[1, 0].plot(
                        ...
                        if PS.axle_roll == "Show":
                            ...
                            self.axV[1, 0].plot(
                                [x1, x2], [y1, y2], color=rollColor
                            )  # Front axle roll axis
                        ...
                        if PS.body_roll == "Show":
                            ...
                            self.axV[1, 0].plot(
                                [x1, x2], [y1, y2], color=bodyRollColor
                            )  # Body roll axis
                        ...
                        if PS.anti_100 == "Show":
                            ...
                            self.axV[1, 0].plot(
                                [x1, x2], [y1, y2], color=anti100Color
                            )  # Front 100% anti line
                            x1 = 0
                        ...
                        if PS.ride_anti == "Show":
                            ...
                            y2 = outputs.F.IC[S.sample_points, z]
                            self.axV[1, 0].plot(
                                [x1, x2], [y1, y2], color=rideAntiColor
                            )  # Front ride anti line
                            x1 = 0
                            x2 = outputs.R.IC[S.sample_points, x]
                            y1 = 0
                            y2 = outputs.R.IC[S.sample_points, z]
                        ...
                        if PS.roll_center == "Show":
                            ...
                            self.axV[1, 0].plot(
                                ...
                                markersize=markerSize,
                                color=rollColor,
                            )  # Rear ride roll center

                        if PS.ride_IC == "Show":
                            self.axV[1, 0].plot(
                                outputs.F.IC[S.sample_points, x],
                                outputs.F.IC[S.sample_points, z],
                                marker="x",
                                color=ICcolor,
                                markersize=markerSize,
                            )  # Front ride IC
                            self.axV[1, 0].plot(
                                outputs.R.IC[S.sample_points, x],
                                outputs.R.IC[S.sample_points, z],
                                marker="x",
                                color=ICcolor,
                                markersize=markerSize,
                            )  # Rear ride IC
                        ...
                        self.axV[1, 0].set_xlim(
                            [
                                -1.1 * inputs.R.tire_diameter / 2,
                                1.1 * inputs.F.tire_diameter / 2 + inputs.V.wheelbase,
                            ]
                        )
                        self.axV[1, 0].set_ylim(
                            [
                                0,
                                1.1
                                * max(inputs.F.tire_diameter, inputs.R.tire_diameter),
                            ]
                        )
                    ...
                    if True:  # Side View Travel
                        if True:  # Ghost car
                            carX = [
                                ...
                                inputs.V.wheelbase
                                + inputs.F.tire_diameter / 2
                                + inputs.F.tire_diameter * 0.1,
                                inputs.V.wheelbase
                                + inputs.F.tire_diameter / 2
                                + inputs.F.tire_diameter * 0.1,
                                inputs.V.wheelbase - inputs.F.tire_diameter / 2,
                                0.9 * inputs.V.wheelbase - inputs.F.tire_diameter / 2,
                                -inputs.R.tire_diameter / 2
                                - inputs.R.tire_diameter * 0.1,
                                -inputs.R.tire_diameter / 2
                                - inputs.R.tire_diameter * 0.1,
                                -inputs.R.tire_diameter / 2,
                        ...
                        self.axV[1, 1].plot(
                            [x1, x2], [y1, y2], color=upperColor, linewidth=travelLine
                        )  # Front upper bump
                        x1 = outputs.F.UF[x]
                        x2 = outputs.F.UA[0][x]
                        y1 = outputs.F.UF[z]
                        ...
                        self.axV[1, 1].plot(
                            [x1, x2], [y1, y2], color=lowerColor, linewidth=travelLine
                        )  # Front lower bump
                        x1 = outputs.F.LF[x]
                        x2 = outputs.F.LA[0][x]
                        y1 = outputs.F.LF[z]
                        ...
                        self.axV[1, 1].set_xlim(
                            [
                                -1.35 * inputs.R.tire_diameter / 2,
                                1.35 * inputs.F.tire_diameter / 2 + inputs.V.wheelbase,
                            ]
                        )
                        self.axV[1, 1].set_ylim(
                            [
                                0,
                                1.1 * inputs.F.tire_radius
                                + inputs.F.tire_diameter / 1.4
                                + inputs.F.tire_diameter / 2,
                            ]
                        )
                    ...
                    if True:  # Both Roll Axis
                        self.axV[0, 1].plot(
                            outputs.F.Roll_Center,
                            100
                            * (outputs.F.Travel - inputs.F.droop)
                            / (inputs.F.bump - inputs.F.droop),
                            color=plotMainColor,
                        )  # Front roll center
                        self.axV[0, 1].plot(
                            outputs.R.Roll_Center,
                            100
                            * (outputs.R.Travel - inputs.R.droop)
                            / (inputs.R.bump - inputs.R.droop),
                            color=plotSecondaryColor,
                        )  # Rear roll center
                        self.axV[0, 1].plot(
                            outputs.F.Roll_Center[S.sample_points],
                            -100 * inputs.F.droop / (inputs.F.bump - inputs.F.droop),
                            marker="_",
                            markersize=20,
                            color=plotMainColor,
                        )  # Front ride %
                        self.axV[0, 1].plot(
                            outputs.R.Roll_Center[S.sample_points],
                            -100 * inputs.R.droop / (inputs.R.bump - inputs.R.droop),
                            marker="_",
                            markersize=20,
                            color=plotSecondaryColor,
                        )  # Rear ride %
                        ...
                        self.axV[0, 1].set_xlim(
                            [
                                min(
                                    min(outputs.F.Roll_Center),
                                    min(outputs.R.Roll_Center),
                                )
                                - 1,
                                max(
                                    max(outputs.F.Roll_Center),
                                    max(outputs.R.Roll_Center),
                                )
                                + 1,
                            ]
                        )
                        ...
                        self.axV[0, 1].set_xlabel(
                            "Roll Center Height [{}]".format(S.position_units)
                        )
                    ...
                    for i_x in range(2):
                        for i_y in range(2):
                            ...
                            self.axV[i_x, i_y].spines[
                                ["top", "bottom", "left", "right"]
                            ].set_color(entryTextColor)
                            self.axV[i_x, i_y].tick_params(
                                axis="x", colors=entryTextColor
                            )
                            self.axV[i_x, i_y].tick_params(
                                axis="y", colors=entryTextColor
                ...
                if True:  # Update Text Outputs
                    label_FU_Force.config(
                        text="{:.0f} {}".format(outputs.F.U_Max_Force, S.force_units)
                    )
                    label_FL_Force.config(
                        text="{:.0f} {}".format(outputs.F.L_Max_Force, S.force_units)
                    )
                    if inputs.F.panhard:
                        label_FP_Force.config(
                            text="{:.0f} {}".format(
                                outputs.F.P_Max_Force, S.force_units
                            )
                        )
                    else:
                    ...
                    if inputs.F.U_count == 1:
                        ...
                        label_FU_Converge.config(
                            text="{:.0f}\u00b0".format(outputs.F.Upper_Convergence)
                        )
                    label_FL_Converge.config(
                        text="{:.0f}\u00b0".format(outputs.F.Lower_Convergence)
                    )
                    label_FU_3d_length.config(
                        text="{:.1f} {}".format(outputs.F.U_Length_3D, S.position_units)
                    )
                    label_FL_3d_length.config(
                        text="{:.1f} {}".format(outputs.F.L_Length_3D, S.position_units)
                    )
                    if inputs.F.panhard:
                        label_FP_3d_length.config(
                            text="{:.1f} {}".format(
                                outputs.F.P_Length_3D, S.position_units
                            )
                        )
                    else:
                        label_FP_3d_length.config(text="N/A")
                    label_FU_2d_length.config(
                        text="{:.1f} {}".format(outputs.F.U_Length_2D, S.position_units)
                    )
                    label_FL_2d_length.config(
                        text="{:.1f} {}".format(outputs.F.L_Length_2D, S.position_units)
                    )
                    if inputs.F.panhard:
                        label_FP_2d_length.config(
                            text="{:.1f} {}".format(
                                outputs.F.P_Length_2D, S.position_units
                            )
                        )
                    else:
                        label_FP_2d_length.config(text="N/A")
                    label_FU_3d_percent.config(
                        text="{:.1f} %".format(
                            100 * outputs.F.U_Length_3D / outputs.F.L_Length_3D
                        )
                    )
                    label_FL_3d_percent.config(
                        text="{:.1f} %".format(
                            100 * outputs.F.L_Length_3D / outputs.F.U_Length_3D
                        )
                    )
                    label_FU_2d_percent.config(
                        text="{:.1f} %".format(
                            100 * outputs.F.U_Length_2D / outputs.F.L_Length_2D
                        )
                    )
                    label_FL_2d_percent.config(
                        text="{:.1f} %".format(
                            100 * outputs.F.L_Length_2D / outputs.F.U_Length_2D
                        )
                    )
                    if inputs.F.U_count == 1:
                        label_F_converge.config(text="N/A")
                    else:
                        label_F_converge.config(
                            text="{:.1f}\u00b0".format(outputs.F.Total_Convergence)
                        )

                    label_RU_Force.config(
                        text="{:.0f} {}".format(outputs.R.U_Max_Force, S.force_units)
                    )
                    label_RL_Force.config(
                        text="{:.0f} {}".format(outputs.R.L_Max_Force, S.force_units)
                    )
                    if inputs.R.panhard:
                        label_RP_Force.config(
                            text="{:.0f} {}".format(
                                outputs.R.P_Max_Force, S.force_units
                            )
                        )
                    else:
                    ...
                    if inputs.R.U_count == 1:
                        ...
                        label_RU_Converge.config(
                            text="{:.0f}\u00b0".format(outputs.R.Upper_Convergence)
                        )
                    label_RL_Converge.config(
                        text="{:.0f}\u00b0".format(outputs.R.Lower_Convergence)
                    )
                    label_RU_3d_length.config(
                        text="{:.1f} {}".format(outputs.R.U_Length_3D, S.position_units)
                    )
                    label_RL_3d_length.config(
                        text="{:.1f} {}".format(outputs.R.L_Length_3D, S.position_units)
                    )
                    if inputs.R.panhard:
                        label_RP_3d_length.config(
                            text="{:.1f} {}".format(
                                outputs.R.P_Length_3D, S.position_units
                            )
                        )
                    else:
                        label_RP_3d_length.config(text="N/A")
                    label_RU_2d_length.config(
                        text="{:.1f} {}".format(outputs.R.U_Length_2D, S.position_units)
                    )
                    label_RL_2d_length.config(
                        text="{:.1f} {}".format(outputs.R.L_Length_2D, S.position_units)
                    )
                    if inputs.R.panhard:
                        label_RP_2d_length.config(
                            text="{:.1f} {}".format(
                                outputs.R.P_Length_2D, S.position_units
                            )
                        )
                    else:
                        label_RP_2d_length.config(text="N/A")
                    label_RU_3d_percent.config(
                        text="{:.1f} %".format(
                            100 * outputs.R.U_Length_3D / outputs.R.L_Length_3D
                        )
                    )
                    label_RL_3d_percent.config(
                        text="{:.1f} %".format(
                            100 * outputs.R.L_Length_3D / outputs.R.U_Length_3D
                        )
                    )
                    label_RU_2d_percent.config(
                        text="{:.1f} %".format(
                            100 * outputs.R.U_Length_2D / outputs.R.L_Length_2D
                        )
                    )
                    label_RL_2d_percent.config(
                        text="{:.1f} %".format(
                            100 * outputs.R.L_Length_2D / outputs.R.U_Length_2D
                        )
                    )
                    if inputs.R.U_count == 1:
                        label_R_converge.config(text="N/A")
                    else:
                        label_R_converge.config(
                            text="{:.1f}\u00b0".format(outputs.R.Total_Convergence)
                        )

                    label_CG_ride.config(
                        text="{:.1f} {}".format(
                            outputs.V.Sprung_CG_height[z], S.position_units
                        )
                    )
                    label_CG_body.config(
                        text="{:.1f} {}".format(
                            outputs.V.Sprung_CG_height_Above_Roll_Axis, S.position_units
                        )
                    )
                    label_CG_rear.config(
                        text="{:.1f} {}".format(
                            outputs.R.Sprung_CG_height_Above_Roll_Center,
                            S.position_units,
                        )
                    )
                    label_CG_front.config(
                        text="{:.1f} {}".format(
                            outputs.F.Sprung_CG_height_Above_Roll_Center,
                            S.position_units,
                        )
                    )
                    if outputs.V.Roll_Axis > 0:
                        ...
                        temp_text = ""
                    label_body_roll.config(
                        text="{:.1f}\u00b0 - {}".format(outputs.V.Roll_Axis, temp_text)
                    )
                    label_side_flip.config(
                        text="{:.1f}\u00b0".format(outputs.V.Side_Roll_Angle)
                    )
                    label_climb_flip.config(
                        text="{:.1f}\u00b0".format(outputs.V.Climb_Angle)
                    )
                    label_descent_flip.config(
                        text="{:.1f}\u00b0".format(outputs.V.Descent_Angle)
                    )
